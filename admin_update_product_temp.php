<?php
    ...
    if(isset($_POST['update_product'])){
        ...
        $update_product = $conn->prepare("UPDATE `products` SET name = ? , catagory = ? , details = ? , price = ? WHERE id = ?");
        $update_product->execute([$name, $catagory, $details, $price, $pid]);

        $message[] = 'Successfull product update!';
        if(!empty($image)){
            if($image_size > 2000000){
                ...
                if($update_image){
                    ...
                    $message[] = 'Successfull image update!';
                }       
            }
        }
    }
